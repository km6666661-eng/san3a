...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (!_accountTypeResolved) {
      ...
      _accountType =
          widget.accountType ?? (routeArg is AccountType ? routeArg : null);
  ...
  void _login(BuildContext context) async {
    ...
    try {
      ...
      // After sign-in, resolve the user's profile to determine account type.
      ...
      if (current != null) {
        final profile = await firestoreService.getUserProfile(current.uid);
        if (profile != null && profile.accountType == 'technician') {
          targetRoute = AppRoutes.technicianHome;
        }
      }
      ...
      setState(() {
        if (errorMsg.contains('لا يوجد حساب') ||
            errorMsg.contains('user-not-found')) {
          _emailError = errorMsg;
        } else if (errorMsg.contains('غير صحيحة') ||
        ...
        body: Column(
          children: [
            ...
            Container(
              ...
              decoration: const BoxDecoration(color: Color(0xFF1557D0)),
              child: SafeArea(
                bottom: false,
                child: Padding(
